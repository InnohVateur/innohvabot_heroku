...
module.exports = async client => {
    (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async cmdFile => {
        ...
        if(!cmd.name || !cmd.description && cmd.type != 'USER') return Logger.warn(`Command not loaded : There is no name/description (at ${cmdFile})`);
        if(!cmd.permissions && cmd.hasPermissions) return Logger.warn(`Command not loaded : There is no specified permissions (at ${cmdFile})`);
        if(!cmd.category) return Logger.warn(`Command not loaded : There is no specified category (at ${cmdFile})`);

        cmd.permissions.forEach(permission=>{
            if(!permissionList.includes(permission)) {
                return Logger.typo(`Unknown permission ! [${cmdFile}]`)
            }
        })
